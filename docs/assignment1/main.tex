...
In this simple implementation, the function \texttt{swap}\_\texttt{vals} takes as input the address of two integer variables, \texttt{a} and \texttt{b} exchange the value of two variables \texttt{a} and \texttt{b} first storing the value of one \texttt{a} in a temporary variable \texttt{temp}. In order to swap the values of two arrays, this function is iteratively applied to each element of two arrays \texttt{A} and \texttt{B} of equal length.
...
For this exercise, a simple \texttt{struct} named \texttt{Student} was defined with elements of type \texttt{std::string} which was used to hold student information. Additionally, a function named \texttt{print\_student} was implemented that takes as input a (constant) \texttt{Student} \texttt{struct} and prints out each of the members. These implementations are used in the main function of the code by instantiating some example \texttt{Student} variables, and then using the \texttt{print\_student} function to print the information they were initialized with.
